...
App routing means letting ModelBoard route a specific application's API requests through the local routing service.
...
When routing is enabled, ModelBoard modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, ModelBoard will:
...
Before enabling routing, ModelBoard backs up the original configuration and restores it when disabled.
