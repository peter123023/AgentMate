...
App routing means letting AgentMate route a specific application's API requests through the local routing service.
...
When routing is enabled, AgentMate modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, AgentMate will:
...
Before enabling routing, AgentMate backs up the original configuration and restores it when disabled.
