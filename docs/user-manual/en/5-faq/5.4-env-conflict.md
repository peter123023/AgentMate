...
AgentMate automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with AgentMate configuration
[Expand] [Dismiss]
```
...
3. AgentMate will automatically back up and remove the selected variables
...
If you prefer not to use AgentMate to remove variables, you can handle them manually:
...
- AgentMate provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use AgentMate to manage configurations**: Avoid setting API keys in system environment variables
