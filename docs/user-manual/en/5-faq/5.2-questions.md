...
ModelBoard for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/model-board/releases).
...
2. Add ModelBoard to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.model-board/` directory exists
2. Restore from backup: `~/.model-board/backups/`
...
**Solutions**:
1. Confirm the file was exported by ModelBoard
...
**Windows**:
- Check taskbar settings to ensure the ModelBoard icon is not hidden
...
2. Restart the app
3. Delete `~/.model-board/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask model-board`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and ModelBoard runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, ModelBoard covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `modelboard://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/farion1231/model-board/issues)
...
ModelBoard stores logs under its application configuration directory. By default this is `.model-board` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.model-board/logs/model-board.log` and its rotated files
- Application crashes: `~/.model-board/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.model-board\...`
