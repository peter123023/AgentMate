...
AgentMate for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/peter123023/llm-switch/releases).
...
2. Add AgentMate to your antivirus software's whitelist
...
**Solutions**:
1. Confirm the file was exported by AgentMate
...
**Windows**:
- Check taskbar settings to ensure the AgentMate icon is not hidden
...
3. If using Homebrew: `brew upgrade --cask agentmate`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and AgentMate runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, AgentMate covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `agentmate://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/peter123023/llm-switch/issues)
...
AgentMate stores logs under its application configuration directory. By default this is `.agentmate` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.
...
- The default Windows location is `C:\Users\<username>\.agentmate\...`
