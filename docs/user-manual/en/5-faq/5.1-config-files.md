...
## ModelBoard Data Storage

### Storage Directory

Default location: `~/.model-board/`
...
```
~/.model-board/
├── model-board.db      # SQLite database (SSOT)
...
`model-board.db` is a SQLite database that stores:
...
Hermes uses YAML configuration. ModelBoard writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to ModelBoard's "providers") |
...
ModelBoard's priority when modifying configurations:

1. **ModelBoard Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by ModelBoard)
- ModelBoard's `settings.json`

### Not Recommended to Edit Manually

- `model-board.db` database file
- Backup files
...
ModelBoard v3.7.0 migrated from JSON files to SQLite:
