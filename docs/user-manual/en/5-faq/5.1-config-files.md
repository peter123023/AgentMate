...
Hermes uses YAML configuration. AgentMate writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to AgentMate's "providers") |
...
AgentMate's priority when modifying configurations:

1. **AgentMate Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by AgentMate)
- AgentMate's `settings.json`
...
AgentMate v3.7.0 migrated from JSON files to SQLite:
