...
Only download ModelBoard from **[modelboard.io](https://modelboard.io)**, **[GitHub Releases](https://github.com/farion1231/model-board/releases)**, or the project source repository. Any "ModelBoard" site or client that asks for payment, top-ups, or login credentials is not official.
...
The CLI tools managed by ModelBoard (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
1. Visit the [Releases page](https://github.com/farion1231/model-board/releases)
...
2. Open the DMG, or extract the zip to get `ModelBoard.app`
3. Drag it to the Applications folder

### Signed and Notarized

ModelBoard for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch ModelBoard:

1. The app window displays correctly
2. A ModelBoard icon appears in the system tray
3. The app switcher shows enabled managed apps, and you can switch to the target app panel

## Auto Update

ModelBoard includes built-in auto-update functionality:
...
- Move `ModelBoard.app` to Trash
- Optional: Delete the configuration directory `~/.model-board/`

### Linux

```bash
# Debian/Ubuntu
sudo apt remove model-board

# ArchLinux
paru -R model-board-bin
```
