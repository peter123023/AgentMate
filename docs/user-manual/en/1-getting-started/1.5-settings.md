...
This section describes how to configure ModelBoard according to your preferences.
...
ModelBoard supports four languages:
...
When enabled, ModelBoard automatically runs when the system starts.
...
Starting from v3.13.0, ModelBoard adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger ModelBoard via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If ModelBoard runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, ModelBoard automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that ModelBoard uses when opening a terminal.
...
The storage location for ModelBoard's own data, defaulting to `~/.model-board/`.
...
The exported file name format is `model-board-export-{timestamp}.sql`.
...
Configure ModelBoard's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, ModelBoard automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `model-board-sync`) |
...
- ModelBoard automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, ModelBoard displays details about the remote snapshot:
- Protocol version (v2)
...
There is no local callback port. Authentication and model requests use ModelBoard's global outbound proxy configuration.

The xAI integration does **not** use an OAuth application registered to ModelBoard. It reuses the public OAuth client identity and scopes registered for the official Grok CLI (`client_id: b1a00492-073a-47ea-816f-4c329264a828`, including `grok-cli:access`) to obtain account-backed API access.
...
Displays the current ModelBoard version number, with support for:
