...
## ModelBoard のデータストレージ

### ストレージディレクトリ

デフォルトの場所：`~/.model-board/`
...
```
~/.model-board/
├── model-board.db      # SQLite データベース（SSOT）
...
`model-board.db` は SQLite データベースで、以下を保存しています：
...
Hermes は YAML 設定を使用します。ModelBoard は MCP サーバーを `mcp_servers` に書き込み、編集可能なプロバイダーエントリを `custom_providers` に書き込み、Hermes の `providers` dict にある読み取り専用エントリを読み取り、プロバイダー切り替え時に `model.provider` / `model.default` を更新します。
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（ModelBoard の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
ModelBoard が設定を変更する際の優先順位：

1. **ModelBoard データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（ModelBoard がバックフィルする）
- ModelBoard の `settings.json`

### 手動編集を推奨しないもの

- `model-board.db` データベースファイル
- バックアップファイル
...
ModelBoard v3.7.0 で JSON ファイルから SQLite に移行しました：
