...
/// provider is routed through AgentMate.  A dedicated id is an ownership
...
pub const CC_SWITCH_CODEX_OFFICIAL_PROXY_PROVIDER_ID: &str = "agentmate-official";
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "agentmate-model-catalog.json";
/// Catalog filename written by the pre-rename (model-board) build. Kept so a
/// pointer left in `~/.codex/config.toml` is still recognized as ours and
/// upgraded to the current filename instead of being mistaken for a
/// user-managed external catalog and left dangling.
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME_LEGACY: &str = "model-board-model-catalog.json";
const CODEX_PROXY_AUTH_PLACEHOLDER: &str = "PROXY_MANAGED";

/// Whether `path`'s file name is a catalog file this app owns (current or
/// pre-rename). User-chosen names and paths are deliberately excluded.
fn is_cc_switch_model_catalog_file(path: &str) -> bool {
    matches!(
        Path::new(path).file_name().and_then(|name| name.to_str()),
        Some(CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME)
            | Some(CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME_LEGACY)
    )
}
...
/// this per the vendors' official Codex docs. Also doubles as agentmate's
...
struct CodexManagedOAuthLiveAuthMarker {
    version: u32,
    /// agentmate 本地托管账号 ID，用于区分同一 ChatGPT workspace 下的登录。
    account_id: String,
...
/// Rollback point for the agentmate-owned model catalog. Catalog projection
...
///
/// - `ProxyChat`: agentmate's proxy takes over and converts Responses<->Chat,
...
pub enum CodexCatalogToolProfile {
    ProxyChat,
    NativeResponses,
    /// Codex talks (through agentmate's proxy) to a native Anthropic Messages
...
/// 作为稳定的所有权谓词；agentmate 的本地账号 ID 单独记录在 marker 中。
...
/// Build the native-shaped ChatGPT auth bundle shared by agentmate and Codex CLI.
...
///
/// 删除谓词同时校验 agentmate marker 中的本地账号 ID 与原生 auth.json 中的
...
/// 判断给定的 Codex `auth` 是否属于指定的 agentmate 本地托管账号。
///
/// 原生 `tokens.account_id` 是 workspace ID，可能被多个本地账号共享；因此必须同时
/// 命中 agentmate marker 中的本地账号 ID，不能只按 auth.json 内容判断。
///
...
/// network request. Codex CLI does not share agentmate's process lock, so this
...
/// catalog (models.json) that agentmate mirrors verbatim. Matched against
...
/// runs through agentmate's converter (gpt-5.5 template contract) and the
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(_) => {
            // Only claim the pointer when it is absent or already agentmate-owned.
            ...
            let is_cc_switch_owned = doc
                ...
                .and_then(|item| item.as_str())
                .map(|path| is_cc_switch_model_catalog_file(path))
                .unwrap_or(true);
...
/// but only when it carries agentmate's own `"disabled"` sentinel, so switching
...
/// lifecycle is bound to the agentmate catalog pointer so the field is set/cleaned
...
/// agentmate–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// agentmate–generated file (identified by filename
/// `agentmate-model-catalog.json`). A user-managed external catalog file is
...
/// Read the agentmate Codex model catalog file with a size cap.
...
/// Given `config.toml` text, resolve the on-disk path of the agentmate–owned
...
pub(crate) fn resolve_cc_switch_catalog_path(
    ...
    let is_cc_switch_owned = referenced_path
        ...
        .is_some_and(is_cc_switch_model_catalog_file);
    if !is_cc_switch_owned {
        return None;
    }
    ...
    // /etc）能让 `link/agentmate-model-catalog.json` 通过上面的检查，读取却
...
/// agentmate's own record (extract/backfill), never read by Codex.
...
/// agentmate-owned provider id used by the legacy-shape normalization below.
/// Not a Codex reserved id, so an injected token lands inside the table.
const CODEX_MIGRATED_PROVIDER_ID: &str = "agentmate";

/// Pick the first free agentmate-owned provider id (`agentmate`,
/// `agentmate-2`, …) so migrations never overwrite a user-authored table.
...
/// means "switch reports success, Codex refuses to start" — older agentmate
...
/// table is renamed losslessly to the first free agentmate id (nothing
...
/// ("provider name must not be empty"). Historic agentmate updates and
...
/// a custom provider table named `agentmate`. Before Codex 0.149 this shape
...
pub fn apply_codex_official_proxy_route(
    ...
    // Clean only AgentMate's placeholder from every stale provider table. Real
...
/// Whether a live Codex config is the official route projected by AgentMate.
...
/// Remove only the official takeover route owned by AgentMate. This is a
...
fn plan_codex_live_write(
    ...
    if category == Some("official") {
        // Official configs seeded by older agentmate versions can carry
        ...
        // idle leftovers from older agentmate versions.
    ...
    // into a agentmate-owned custom table before the safety gates run.
...
"#;
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"agentmate-model-catalog.json\"\n";
...
"#;
        ...
        assert!(
            normalized.contains("model_provider = \"agentmate\""),
            "routing must move to the agentmate table; got:\n{normalized}"
        );
        assert!(
            normalized.contains("[model_providers.agentmate]"),
        ...
        assert!(
            ...
            "token must land inside the agentmate table; got:\n{injected}"
        );
        assert_eq!(
    ...
    fn legacy_reroute_normalization_never_overwrites_a_user_cc_switch_table() {
        // A user-authored [model_providers.agentmate] proves nothing about
...
"#;
        ...
        assert!(
            normalized.contains("model_provider = \"agentmate-2\"")
                && normalized.contains("[model_providers.agentmate-2]"),
    ...
    fn stale_reserved_tables_are_renamed_with_fallback_aware_routing() {
        // Older agentmate takeover projections created reserved
...
"#;
        ...
        assert!(
            keyless.contains("model_provider = \"agentmate\"") && keyless.contains("own-key"),
...
"#;
        let local = prepare_codex_provider_live_config(&json!({}), unauthenticated_stale)
            .expect("prepare live config without token");
        assert!(
            local.contains("model_provider = \"agentmate\"")
...
"#;
        let scoped = prepare_codex_provider_live_config(&json!({}), scoped_token_stale)
            .expect("prepare live config without token");
        assert!(
            scoped.contains("model_provider = \"agentmate\"")
...
"#;
        ...
        assert!(
            snapped.contains("model_provider = \"openai\"")
                ...
                && snapped.contains("[model_providers.agentmate]"),
        ...
        assert!(
            official.contains("model_provider = \"openai\"")
                && official.contains("[model_providers.agentmate]"),
...
"#;
        ...
        assert!(
            !cleaned.contains("[model_providers.ollama]")
                ...
                && cleaned.contains("[model_providers.agentmate]")
                && cleaned.contains("[model_providers.agentmate-2]")
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/agentmate-model-catalog.json");
...
"#;
    ...
    fn native_web_search_field_removes_own_sentinel_when_not_disabled() {
        ...
        // removing agentmate's own "disabled" sentinel.
...
"#;
        ...
        assert!(
            ...
            "agentmate's disabled sentinel should be removed when not native"
        );
    }
    ...
    fn native_web_search_field_preserves_user_value() {
        ...
        // only agentmate's "disabled" sentinel is owned/removable.
...
"#;
    ...
    fn resolve_catalog_path_accepts_cc_switch_owned_file() {
        let base = PathBuf::from("/tmp/.codex");
        let config = r#"model_catalog_json = "/tmp/.codex/agentmate-model-catalog.json"
"#;
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as agentmate would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\agentmate-model-catalog.json");
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/agentmate-model-catalog.json");
    ...
    fn set_catalog_json_none_removes_cc_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/agentmate-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove agentmate-owned field regardless of path format"
        );
    }
...
"#;
    ...
    fn set_catalog_json_some_preserves_user_owned_catalog() {
        // When AgentMate generates a catalog (Some arm), it must still respect a
        // user-managed external catalog file instead of clobbering it with the
        // agentmate-owned filename. Only an absent or agentmate-owned pointer is
...
"#;
    ...
    fn resolve_catalog_rejects_absolute_path_outside_config_dir() {
        let config_text = r#"model_catalog_json = "/tmp/secret/agentmate-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_accepts_absolute_path_inside_config_dir() {
        let config_text = r#"model_catalog_json = "/home/user/.codex/agentmate-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_traversal_to_parent_directory() {
        let config_text = r#"model_catalog_json = "../agentmate-model-catalog.json"
"#;
        let base_dir = PathBuf::from("/home/user/.codex");
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
    ...
    fn resolve_catalog_rejects_symlink_escaping_config_dir() {
        ...
        // "link/agentmate-model-catalog.json" 词法上在 base 内，真实读取却落到
        ...
        let config_text = r#"model_catalog_json = "link/agentmate-model-catalog.json"
"#;
        let result = resolve_cc_switch_catalog_path(config_text, &base_dir);
        assert_eq!(
            result, None,
