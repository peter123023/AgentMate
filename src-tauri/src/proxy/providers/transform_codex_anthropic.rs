...
pub(crate) const ANTHROPIC_THINKING_ENCRYPTED_PREFIX: &str = "agentmate-anthropic-thinking-v1:";
// 兼容改名前（cc-switch 时代）已持久化到数据库的旧前缀 thinking 内容
pub(crate) const ANTHROPIC_THINKING_ENCRYPTED_PREFIX_LEGACY: &str = "modelboard-anthropic-thinking-v1:";
pub(crate) const ANTHROPIC_THINKING_ENCRYPTED_PREFIX_LEGACY_CC_SWITCH: &str = "ccswitch-anthropic-thinking-v1:";
...
pub(crate) fn decode_anthropic_thinking_block(encrypted_content: &str) -> Option<Value> {
    let encoded = encrypted_content
        ...
        .or_else(|| encrypted_content.strip_prefix(ANTHROPIC_THINKING_ENCRYPTED_PREFIX_LEGACY_CC_SWITCH))?;
