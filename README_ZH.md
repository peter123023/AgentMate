...
### 🌐 唯一官方网站：**[modelboard.io](https://modelboard.io)**
...
- **数据库**：`~/.model-board/model-board.db`（SQLite — 供应商、MCP、提示词、技能）
- **本地设置**：`~/.model-board/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.model-board/backups/`（自动轮换，保留最近 10 个）
- **SKILLS**：`~/.model-board/skills/`（默认通过软链接连接到对应应用）
- **技能备份**：`~/.model-board/skill-backups/`（卸载前自动创建，保留最近 20 个）
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.model-board/model-board.db`（SQLite）
