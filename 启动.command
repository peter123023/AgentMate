...
APP="./src-tauri/target/release/ModelBoard"
DEBUG_APP="./src-tauri/target/debug/ModelBoard"
...
if pgrep -x ModelBoard >/dev/null 2>&1; then
  echo "==> 发现已在运行的 ModelBoard，正在退出旧实例..."
  pkill -TERM -x ModelBoard 2>/dev/null
  for _ in $(seq 1 20); do
    pgrep -x ModelBoard >/dev/null 2>&1 || break
    sleep 1
  done
  if pgrep -x ModelBoard >/dev/null 2>&1; then
    echo "==> 仍未退出，强制结束"
    pkill -KILL -x ModelBoard 2>/dev/null
    sleep 2
  fi
  echo "==> 旧实例已清理"
fi

echo "==> 启动 ModelBoard（自包含模式，无需 Vite）"
echo "    可执行文件: $APP"
echo
