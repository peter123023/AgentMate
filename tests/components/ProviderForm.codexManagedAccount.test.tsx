...
/**
 * 预设按厂商折叠分组：先展开 OpenAI 厂商组，再在 Popover 内点选 "OpenAI Official"。
 * 组按钮与子项的 icon title 都含 "OpenAI"，必须在 Popover（role=dialog）内查询子项。
 */
async function selectCodexOfficialPreset() {
  fireEvent.click(screen.getByRole("button", { name: /OpenAI/ }));
  const popover = await screen.findByRole("dialog");
  fireEvent.click(
    within(popover).getByRole("button", { name: /OpenAI Official/ }),
  );
}
...
describe("ProviderForm Codex Official managed account", () => {
  ...
  it("persists the selected managed account while stripping OAuth secrets", async () => {
    ...
    await selectCodexOfficialPreset();
    fireEvent.click(
