...
### 🌐 唯一の公式サイト：**[modelboard.io](https://modelboard.io)**
...
- **データベース**: `~/.model-board/model-board.db`（SQLite -- プロバイダ、MCP、Prompts、Skills）
- **ローカル設定**: `~/.model-board/settings.json`（デバイスレベルの UI 設定）
- **バックアップ**: `~/.model-board/backups/`（自動ローテーション、最新 10 件を保持）
- **Skills**: `~/.model-board/skills/`（デフォルトでシンボリックリンクにより対応アプリに接続）
- **Skill バックアップ**: `~/.model-board/skill-backups/`（アンインストール前に自動作成、最新 20 件を保持）
...
- **SSOT** (Single Source of Truth): すべてのデータを `~/.model-board/model-board.db`（SQLite）に集約
